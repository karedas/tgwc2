...
//import routes
require('./routes/profile.js')(logger, app, io);
...
function SocketServer() {
	...
	function ConnectToGameServer(websocket, from_host, code_itime, code_headers, account_id) {
		...
		let dataEventName = 'auth';

		let port =  process.env.SERVER_GAME_PORT;

		let tgconn = net.connect(port, function(){
		});
		...
		function sendToClient(msg) {
			if(msg.toString().indexOf('loginok') != -1) {
				...
				websocket.emit('auth', msg.toString());
		...
		function sendToClientLoggedIn(msg) {
